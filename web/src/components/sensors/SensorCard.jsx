...
export function SensorCard({ sensor }) {
  const lastReading = sensor.last_reading || sensor;
    ...
    <Link to={`/sensors/${sensor.serial_number}`}>
      <Card className="hover:shadow-md transition-shadow cursor-pointer">
        <div className="flex items-center justify-between mb-4">
          ...
          <BatteryBadge level={lastReading.battery_level} />
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{formatTemperature(lastReading.temperature)}</span>
          </div>

          <div className="flex items-center gap-2">
            <Droplets className="w-4 h-4 text-blue-500" />
            <span className="text-gray-600">Umidade:</span>
            <span className="font-medium">{formatHumidity(lastReading.humidity)}</span>
          </div>

          {lastReading.received_at && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Clock className="w-4 h-4" />
              <span>{formatRelativeTime(lastReading.received_at)}</span>
            </div>
          )}
        </div>
      </Card>
